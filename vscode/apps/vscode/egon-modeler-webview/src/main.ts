...
/**
 * Error thrown when EgonClient is not initialized.
 */
...
/**
 * Get the EgonClient instance.
 * @throws NoClientError if the client is not initialized
 */
...
/**
 * Import a story into the modeler.
...
/**
 * Debounced import to avoid too many updates.
 */
const updateStory = debounce(importStory, 100);

/**
 * Export the current story from the modeler.
 * @returns JSON string of the domain story document
 */
...
/**
 * Send the changed story to the backend to update the .egn file.
 */
...
/**
 * Initialize the domain story modeler.
 * @param story JSON string of the domain story document
...
async function initializeDomainStoryModeler(story: string, state: WebviewState) {
    ...
    // Subscribe to story changes (EgonClient debounces internally at 100ms)
    egonClient.on("story.changed", sendStoryChanges);

    // Subscribe to viewport changes to persist viewport state
 ...
 * There are two reasons why a webview gets built:
...
const emptyStory = JSON.stringify({
    domain: {
        ...
        workObjects: {
            ...
            Conversation:
                '<svg height="48" viewBox="0 0 24 26" width="24" xmlns="http://www.w3.org/2000/svg"><path d="M0 0h24v24H0z" fill="none"/><path d="M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm0 14H6l-2 2V4h16v12z"/></svg>',
